                                ...
                                <div class="product-image">
                                    <div class="product_img_box">
                                        @empty($products->images->first()->thumbnail)
                                            <img id="product_img" src="{{ asset('assets/images/product/default.png') }}"
                                                data-zoom-image="{{ asset('assets/images/product/default.png') }}"
                                                alt="{{ $products->name }}" />
                                        ...
                                        @else
                                            ...
                                            @endphp
                                            <img id="product_img" src="{{ asset('storage/galerie') }}/{{ $images[0] }}"
                                                data-zoom-image="{{ asset('storage/galerie') }}/{{ $images[0] }}"
                                                alt="{{ $products->name }}" />
                                            <a href="#" class="product_img_zoom" title="Zoom">
                                                <span class="linearicons-zoom-in"></span>
                                            </a>
                                    ...
                                    <div wire:ignore id="pr_item_gallery" class="product_gallery_item slick_slider"
                                        data-slides-to-show="4" data-slides-to-scroll="1" data-infinite="false">

                                        @empty($images)
                                            <div class="item">
                                                <a href="#" class="product_gallery_item active"
                                                    data-image="{{ asset('assets/images/product/default.png') }}"
                                                    data-zoom-image="{{ asset('assets/images/product/default.png') }}">
                                        ...
                                        @else
                                            @foreach ($images as $image)
                                                <div class="item">
                                                    <a href="#" class="product_gallery_item active"
                                                        data-image="{{ asset('storage/galerie') }}/{{ $image }}"
                                                        data-zoom-image="{{ asset('storage/galerie') }}/{{ $image }}">
                                        ...
                                        <ul class="nav nav-tabs" role="tablist">
                                            ...
                                            <li class="nav-item">
                                                <a class="nav-link" id="Reviews-tab" data-bs-toggle="tab"
                                                    ...
                                                    ({{ $products->reviews->where('product_id', $products->id)->count() }})</a>
                                            </li>
                                        </ul>
                                        <div class="tab-content shop_info_tab">
                                            ...
                                            <div class="tab-pane fade" id="Reviews" role="tabpanel"
                                                ...
                                                @if (Session::has('message_r'))
                                                    <div class="alert alert-success">{{ Session::get('message_r') }}
                                                    </div>
                                                @endif
                                                <div class="comments">
                                                    <h5 class="product_tab_title">
                                                        {{ $products->reviews->where('product_id', $products->id)->count() }}
                                                        Review For <span>{{ $products->name }}</span></h5>
                                                    <ul class="list_none comment_list mt-4">
                                                        @foreach ($products->reviews->where('product_id', $products->id) as $review)
                                                            <li>
                                                                <div class="comment_img">
                                                                    <img src="{{ asset('assets/site/assets/images/user1.jpg') }}"
                                                                        alt="user1" />
                                                                </div>
                                                                <div class="comment_block">
                                                                    ...
                                                                    <p class="customer_meta">
                                                                        <span
                                                                            class="review_author">{{ $review->name }}</span>
                                                                        <span
                                                                            class="comment-date">{{ Carbon\Carbon::parse($review->created_at)->format('d F Y g:i A') }}</span>
                                                                    </p>
                                        ...
                                        <div class="product_box text-center">
                                            <div class="div_image product_img">
                                                @empty($products_relateds->images->first()->thumbnail)
                                                    <a
                                                        href="{{ route('site.detail-produit', ['id' => $products_relateds->id]) }}">
                                                        <img src="{{ asset('assets/images/product/default.png') }}"
                                                            alt="{{ $products_relateds->name }}">
                                                    </a>
                                                @else
                                                    @php
                                                    ...
                                                    <a
                                                        href="{{ route('site.detail-produit', ['id' => $products_relateds->id]) }}">
                                                        <img src="{{ asset('storage/galerie') }}/{{ $images[0] }}"
                                                            alt="{{ $products_relateds->name }}">
                                                    </a>
                                                @endempty
                                                <div class="product_action_box">
                                                    ...
                                                    class="icon-magnifier-add"></i></a></li> --}}
                                                        @if ($witems->contains($products_relateds->id))
                                                            <li><a href="#"
                                                                    wire:click.prevent="removeFromWishList({{ $products_relateds->id }})"><i
                                                                        class="icon-heart"
                                                                        style="color: red;"></i></a></li>
                                                        @else
                                                            <li><a href="#"
                                                                    wire:click.prevent="addToWishlist({{ $products_relateds->id }},'{{ $products_relateds->name }}',{{ $products_relateds->normal_price }})"><i
